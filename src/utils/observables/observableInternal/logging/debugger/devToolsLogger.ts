...
import { IObservable, IObserver, TransactionImpl } from '../../base';
...
import { deepAssign, deepAssignDeleteNulls, getFirstStackFrameOutsideOf, Throttler } from './utils';
...
export class DevToolsLogger implements IObservableLogger {
		...
		return {
			...
			requests: {
				...
				getDerivedInfo: instanceId => {
					...
					return {
						dependencies: [...d['dependencies']].map(d => this._formatObservable(d)),
						observers: [...d['observers']].map(d => this._formatObserver(d)).filter(isDefined),
					};
				},
				getAutorunInfo: instanceId => {
					const obs = this._aliveInstances.get(instanceId) as AutorunObserver;
					return {
						dependencies: [...obs['dependencies']].map(d => this._formatObservable(d)),
					};
				},
				getTransactionState: () => {
	...
	private getTransactionState(): ITransactionState | undefined {
		...
		const observerQueue = txs.flatMap(t => t['updatingObservers'] ?? []).map(o => o.observer);
	...
	private _getInfo(observer: IObserver, queue: (observer: IObserver) => void): ObserverInstanceState | undefined {
		if (observer instanceof Derived) {
			const observersToUpdate = [...observer['observers']];
			for (const o of observersToUpdate) {
				queue(o);
			}

			const info = this._getObservableInfo(observer)!;
			const base = { name: observer.debugName, instanceId: info.instanceId, updateCount: observer['updateCount'] };
	...
	private _getDeclarationId(type: IObsDeclaration['type']): number {
		...
		const loc = getFirstStackFrameOutsideOf(stack, /[/\\]observableInternal[/\\]|[/\\]util(s)?\./);
